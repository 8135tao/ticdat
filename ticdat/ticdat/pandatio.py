...
class DataFramePanFactory(freezable_factory(object, "_isFrozen")):
    """
    Primary class for reading/writing csv files with PanDat objects.
    Don't create this object explicitly. A CsvPanFactory will
    automatically be associated with the csv attribute of the parent
    ...
    def create_pan_dat(self, tbl_names, fill_missing_fields=False, **kwargs):
        ...
        for t, f in tbl_names.items():
            ...
            for col, dtype in kwargs_["dtype"].items():
                # print(f'column {col} has datatype: {dtype}')
                f[col] = f[col].replace('', float('nan')).replace('nan', float('nan')).fillna(float('nan'))
                try:
                    if (dtype == float) & (f[col].dtypes == 'O'):
                        f[col] = f[col].apply(lambda x: x if pd.isnull(x) else ast.literal_eval(x))
                    else:
                        pass
                except Exception:
                    print(f'表{t}中的{col}列数据格式问题！')
                    print("-----------------------------")
                    traceback.print_exc()
                    continue
            rtn[t] = f
